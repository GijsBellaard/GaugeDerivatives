...
from geometry import connection, covariant_derivative, partial_derivative
...
for name, values in (("r^2 cos 2th", radius**2 * torch.cos(2 * angle)),
    ...
    error = laplacian(field, metric, levi_civita).data[INTERIOR].abs().max() / values[INTERIOR].abs().max()
...
check("connection of a flat metric is zero",
      bool((connection(flat).data == 0).all()), f"{tuple(connection(flat).data.shape)}")
...
levi_civita = connection(flat)
gradient = covariant_derivative(field, levi_civita)
outer = tensor_product(gradient, gradient)
...
first = change_basis(gradient, frame)
error = (first.data[..., 0] - torch.einsum("...a,...a->...", gradient.data, frame.data[..., 0])).abs().max()
check("first order gauge derivative = gradient along frame", error < 1e-14, f"{error:.2e}")
...
for name, Q, v, lam in (("eigenframe", (F + F.mT) / 2, frame.data, values.data),
    ...
    error = max((Q @ v - g @ v * lam[..., None, :]).abs().max(), (v.mT @ g @ v - torch.eye(2)).abs().max())
